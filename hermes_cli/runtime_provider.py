...
from hermes_cli.providers import (
    custom_provider_slug, determine_api_mode, is_official_openai_host,
    is_actual_route, is_saved_custom_endpoint, nous_api_mode,
)
...
def _resolve_plain_custom_api_mode(model_cfg: Dict[str, Any], base_url: str) -> str:
    ...
    Actual routes require Chat Completions. Otherwise an explicit mode is authoritative and URL
    detection is only the fallback for the automatic setting, so a relay can intentionally expose
    a different protocol from its hostname.
    """
