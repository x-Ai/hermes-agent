(function () {
  ...
  function localizedTier(t, tier) {
    const labels = t && t.achievements && t.achievements.tier_labels;
    return (labels && labels[tier]) || tier;
  }

  function localizedCategory(t, category) {
    const labels = t && t.achievements && t.achievements.category_labels;
    return (labels && labels[category]) || category;
  }

  function localizedAchievement(t, locale, achievement) {
    if (locale !== "zh") return achievement;
    const namespace = t && t.achievements;
    const catalog = namespace && namespace.catalog;
    const entry = catalog && catalog[achievement.id];
    const secret = achievement.state === "secret";
    const criteriaCopy = namespace && namespace.criteria_localization;
    let criteria = achievement.criteria;
    if (criteriaCopy) {
      criteria = secret ? criteriaCopy.secret : criteriaCopy.matching;
      if (!secret && Array.isArray(achievement.tiers) && achievement.tiers.length) {
    ...
    return Object.assign({}, achievement, {
      name: secret ? achievement.name : ((entry && entry.name) || achievement.name),
      description: secret
        ? "秘密成就：Hermes 在会话历史中检测到首次相关行为后才会揭示。"
        : ((entry && entry.description) || achievement.description),
      category: localizedCategory(t, achievement.category),
      criteria: criteria,
    });
  }
  ...
  async function buildShareImage(achievement, labels) {
    ...
    const nameLines = wrapText(ctx, achievement.name || labels.achievementFallback, rMaxWidth).slice(0, 2);
    let cursorY = 150;
    ...
    // Tier badge pill
    const badgeLabel = labels.tierBadge;
    ...
    const stamp = labels.unlocked;
    const stampW = ctx.measureText(stamp).width;
    ctx.fillText(stamp, W - 70 - stampW, 70);

    return await new Promise(function (resolve, reject) {
      canvas.toBlob(function (blob) {
        if (blob) resolve(blob); else reject(new Error(labels.renderFailed));
      }, "image/png");
    });
  }
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    hooks.useEffect(function () {
      ...
      buildShareImage(achievement, {
        tierBadge: tx(t, "share.image_tier", "{tier} tier", { tier: localizedTier(t, tier) }),
        unlocked: tx(t, "share.image_unlocked", "◆ Unlocked"),
        achievementFallback: tx(t, "share.image_achievement_fallback", "Achievement"),
        renderFailed: tx(t, "share.image_render_failed", "Could not render the achievement image."),
      }).then(function (blob) {
        if (cancelled) return;
    ...
    function tweetText() {
      const tierPart = achievement.tier ? (localizedTier(t, achievement.tier) + " ") : "";
  ...
  function AchievementCard({ achievement }) {
    const { t, locale } = useI18n();
    achievement = localizedAchievement(t, locale, achievement);
    ...
    let tierLabel;
    if (achievement.tier) {
      tierLabel = localizedTier(t, achievement.tier);
    } else if (targetTier) {
      tierLabel = tx(t, "tier.target", "Target {tier}", { tier: localizedTier(t, targetTier) });
  ...
  function AchievementsPage() {
    ...
    const highest = highestRaw ? localizedTier(t, highestRaw) : tx(t, "stats.none_yet", "None yet");
    ...
    return React.createElement("div", { className: "ha-page" },
      ...
      React.createElement("div", { className: "ha-stats" },
        ...
        React.createElement(StatCard, { label: tx(t, "stats.latest", "Latest"), value: latest[0] ? localizedAchievement(t, locale, latest[0]).name : tx(t, "stats.none_yet", "None yet"), hint: latest[0] ? localizedCategory(t, latest[0].category) : tx(t, "stats.latest_hint_empty", "run Hermes more") })
      ),
      ...
      latest.length > 0 && React.createElement("section", { className: "ha-latest" },
        ...
        React.createElement("div", { className: "ha-latest-row" }, latest.map(function (a) {
          return React.createElement("div", { key: a.id, className: cn("ha-chip", tierClass(a.tier)) },
            ...
            localizedAchievement(t, locale, a).name
          );
        }))
      ),
