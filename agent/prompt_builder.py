...
def _run_backend_probe(env_type: str, terminal_tool) -> str:
    ...
    # A system-prompt probe is an observation, not a project workspace. Keep it
    # short-lived and detach it from user mounts, forwarded env, and persistent
    # container identity.
    if env_type in _EPHEMERAL_PROBE_BACKENDS and container_config is not None:
...
def _remote_backend_hint(backend: str, *, environment_probe_enabled: bool = True) -> str:
    ...
    return lead + (
        ...
        f"user, $HOME, and working directory are unknown from here. "
...
def build_environment_hints(*, environment_probe_enabled: bool = True) -> str:
    ...
    hints = (
        [_remote_backend_hint(backend, environment_probe_enabled=environment_probe_enabled)]
        if is_remote_backend
        else _local_host_hints()
    )
