...
agent:
  ...
  output_truncation_retries: 1 # 达到输出上限且无可见文本时的恢复重试次数（0-3）
  post_tool_empty_retries: 1   # 工具调用后无可见文本时的继续提示次数（0-3）
  thinking_prefill_retries: 2  # 仅思考响应的预填充重试次数（0-3）
  empty_response_retries: 3    # 最后的普通空响应重试次数（0-3）
```
...
`agent.output_truncation_retries` 与 API 错误重试相互独立。它仅在提供方明确报告已达到输出 Token 上限、并且尚未生成可见文本时生效。默认值 `1` 会在临时关闭 reasoning 后重试一次，并可能调高较小的隐式传输输出上限；设为 `0` 会立即返回截断提示。最多可重试 `3` 次，每次都可能重新计费完整输入。Desktop 可在**设置 → 高级**中、紧邻“API 重试次数”进行配置。

其余无可见输出的三层恢复分别由 `agent.post_tool_empty_retries`、
`agent.thinking_prefill_retries` 和 `agent.empty_response_retries` 控制。它们按该顺序运行，
默认次数依次为 `1`、`2`、`3`；任意一项设为 `0` 只会关闭对应恢复层。
每次重试都可能重新计费。空响应成本保护可能在达到配置次数前提前停止，
但绝不会增加重试次数。Desktop 和 Dashboard 都在其他重试选项旁显示这三个选择器。
