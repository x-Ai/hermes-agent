...
def _backend_follows_session_workspace(backend: str) -> bool:
    """Whether this backend bind-mounts the current session's selected workspace."""
    ...
    def _flag(env_var: str, cfg_key: str) -> bool:
        ...
        if raw is None:
            raw = _workdir_terminal_cfg(cfg_key)
        return is_truthy_value(raw)

    (mount_env, mount_key), (session_env, session_key) = flag_pair
    return _flag(mount_env, mount_key) and _flag(session_env, session_key)
