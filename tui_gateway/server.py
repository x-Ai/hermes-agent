...
def _get_usage(agent) -> dict:
    ...
    if comp:
        ...
        if ctx_max:
            usage.setdefault("context_max", ctx_max)
            # Compression invalidates the old occupancy until the provider reports the
            # first real prompt size for the new window. Clients use this marker to
            # discard rather than merge the stale pre-compression measurement.
...
def _reportable_provider(provider, *, base_url=None, model=None) -> str:
    """Upgrade bare ``custom`` to the durable configured endpoint identity for UI clients."""
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    info_provider = pending_provider or mirror.get("provider", getattr(agent, "provider", ""))
    info: dict = {
        "model": info_model,
...
def _lazy_resume_info(
    cwd: str, *, model: str = "", provider: str = "", base_url: str = "", profile: str | None = None
) -> dict:
    ...
    return {
        ...
        **({"provider": _reportable_provider(provider, base_url=base_url or None, model=model or None)} if provider else {}),
    }
