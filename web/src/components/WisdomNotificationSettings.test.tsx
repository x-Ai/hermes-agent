...
vi.mock('@/lib/api', () => ({ api: { prepareWisdomMute: prepare, getWisdomMute: read, chooseWisdomMute: choose } }))
const control = {
  id: 'a'.repeat(32),
...
describe('Wisdom notification settings', () => {
  it('loads only on request and changes only after Save', async () => {
    ...
    expect(choose).toHaveBeenCalledWith(control.id, '1_week', 'demo')
  })
  ...
  it('drops responses from the previous profile', async () => {
    ...
    await open()
    expect(prepare).toHaveBeenLastCalledWith('second')
  })
})
