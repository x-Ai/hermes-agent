...
export function OAuthLoginModal({ provider, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(e => {
        if (!isMounted.current) return;
        setPhase("error");
        setErrorMsg(dashboardCopy.startFailed.replace("{error}", errorMessage(e)));
      });
    return () => {
  ...
  useEffect(() => {
    ...
    pollTimer.current = window.setInterval(async () => {
      try {
        ...
        if (!isMounted.current) return;
        setPhase("error");
        setErrorMsg(dashboardCopy.pollingFailed.replace("{error}", errorMessage(e)));
        if (pollTimer.current !== null) window.clearInterval(pollTimer.current);
      }
    }, 2000);
    return () => {
  ...
  const handleSubmitPkceCode = async () => {
    ...
    try {
      ...
      if (!isMounted.current) return;
      setPhase("error");
      setErrorMsg(dashboardCopy.submitFailed.replace("{error}", errorMessage(e)));
    }
  };
                  ...
                  onClick={() => {
                    ...
                    api
                      ...
                      .catch(e => {
                        if (!isMounted.current) return;
                        setPhase("error");
                        setErrorMsg(dashboardCopy.retryFailed.replace("{error}", errorMessage(e)));
                      });
                  }}
                >
                  {t.common.retry}
