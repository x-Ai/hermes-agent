...
import { localizeConfigSection } from "@/i18n/config-metadata";
...
export default function ConfigPage() {
  ...
  function prettyCategoryName(cat: string): string {
    ...
    return localizeConfigSection(cat, locale);
  }
  ...
  const searchMatchedFields = useMemo(() => {
    ...
    return Object.entries(schema).filter(([key, s]) => {
      ...
      const localizedLabel = localizeConfigSection(label, locale);
      const localizedDescription = localizeConfigSection(key, locale);
      return (
      ...
      return (
        <div key={key}>
          ...
          {showSection && (
            <div className="flex items-center gap-2 pt-4 pb-2 first:pt-0">
              <span className="font-mondwest text-display text-xs font-semibold tracking-wider text-muted-foreground">
                {localizeConfigSection(section, locale)}
              </span>
              <div className="flex-1 border-t border-border" />
            </div>
          )}
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <ConfirmDialog
        ...
        description={(
          t.config.resetFieldsDescription ??
          "This will reset {count} field(s) to their default values."
        ).replace("{count}", String((isSearching ? searchMatchedFields : activeFields).length))}
        destructive
