...
export default function AnalyticsPage() {
  ...
  const { t } = useI18n();
  const dashboardCopy = getDashboardCopy(t).analytics;
          ...
          <CardContent className="py-12">
            <div className="mx-auto flex max-w-2xl flex-col gap-3 text-sm text-muted-foreground">
              <h2 className="font-mondwest text-display text-base tracking-wider text-foreground">
                {dashboardCopy.hiddenTitle}
              </h2>
              <p>{dashboardCopy.hiddenEstimate}</p>
              <p>{dashboardCopy.hiddenVariance}</p>
              <p>
                {dashboardCopy.hiddenActionBefore}{" "}
                <span className="font-mono">dashboard.show_token_analytics: true</span>{" "}
                <a href="/config" className="underline">
                  {t.app.nav.config}
                </a>
                {` ${dashboardCopy.hiddenActionAfter}`}
              </p>
            </div>
          </CardContent>
        </Card>
