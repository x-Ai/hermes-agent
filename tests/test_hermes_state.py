...
class TestFTS5Search:
    ...
    def test_search_projection_skips_context_enrichment_queries(self, db, monkeypatch):
        ...
        @contextmanager
        def traced_read_ctx():
            # The read pool may hand each enrichment query a different
            # connection. Trace the connection actually borrowed for every
            # read instead of snapshotting one pool member up front.
