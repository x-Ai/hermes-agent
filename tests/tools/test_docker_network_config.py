...
def test_every_sandbox_creator_passes_the_full_container_config(monkeypatch):
    ...
    the SAME container_config keys, while the factual prompt probe stays ephemeral and unmounted.
    Each used to keep a private (key, default) table and drifted:
    ...
    probe, execution = seen
    assert execution == expected
    assert probe.keys() == execution.keys()
    # Probe isolation deliberately drops workspace and credential forwarding, but must
    # retain the operator's network lockdown and runtime arguments from the shared shaper.
    assert probe["docker_network"] is execution["docker_network"] is False
    assert probe["docker_extra_args"] == execution["docker_extra_args"] == config["docker_extra_args"]
    assert not probe["container_persistent"] and not probe["docker_persist_across_processes"]
    assert not probe["docker_forward_env"] and not probe["docker_env"]
