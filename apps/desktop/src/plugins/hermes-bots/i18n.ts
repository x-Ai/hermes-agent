...
const en: BotsMessages = {
  ...
  group: {
    ...
    activitySlotWait: member => `${member} couldn't start — too many bots are running`,
    activityLabels: {
...
const ja: BotsMessages = {
  ...
  group: {
    ...
    activitySlotWait: member => `${member}を開始できませんでした — 実行中のボットが多すぎます`,
    activityLabels: {
      queued: 'メッセージを送信しました',
      working: '作業中…',
      replied: '返信しました',
      ...
      failed: 'エラーが発生しました',
      cancelled: '新しいメッセージでターンが中断されました',
      settled: 'ターンが完了しました',
      capped: 'ターンまたはメッセージ上限で停止しました',
      delivered: '遅延した返信を配信しました',
      held: '保留中です — @メンションまたは resume で解除できます',
...
const zh: BotsMessages = {
  ...
  group: {
    ...
    activityBy: (member, action) => `${member}${action}`,
    activitySlotWait: member => `${member}无法启动 — 同时运行的智能体过多`,
    activityLabels: {
      queued: '发送了一条消息',
      working: '正在处理…',
      replied: '已回复',
...
const zhHant: BotsMessages = {
  ...
  group: {
    ...
    activityBy: (member, action) => `${member}${action}`,
    activitySlotWait: member => `${member}無法啟動 — 同時執行的智慧體過多`,
    activityLabels: {
      queued: '傳送了一則訊息',
      working: '正在處理…',
      replied: '已回覆',
      ...
      settled: '本回合已結束',
      capped: '達到回合或訊息上限，本回合已停止',
      delivered: '已送達延遲回覆',
      held: '已暫停（由你停止）— 提及它或傳送 resume 即可恢復',
      stopped: '已停止房間 — 其餘回合將保持暫停，直到恢復'
    },
    stop: '停止',
...
const ru: BotsMessages = {
  ...
  group: {
    ...
    activityActorBot: 'Бот',
    activityDidSomething: 'выполнил действие',
    activityBy: (member, action) => `${member} ${action}`,
    activitySlotWait: member => `${member} не запущен — одновременно работает слишком много ботов`,
    activityLabels: {
      queued: 'отправил сообщение',
      working: 'работает…',
      replied: 'ответил',
      passed: 'пропустил',
      timedOut: 'не уложился во время',
      failed: 'столкнулся с ошибкой',
      cancelled: 'ход прерван новым сообщением',
      settled: 'ход завершён',
      capped: 'ход остановлен по лимиту раундов или сообщений',
      delivered: 'доставил запоздалый ответ',
      held: 'приостановлен — упомяните его или отправьте resume для продолжения',
      stopped: 'остановил комнату — остальные ходы приостановлены до возобновления'
    },
    stop: 'Стоп',
...
const ar: BotsMessages = {
  ...
  group: {
    ...
    activityActorBot: 'روبوت',
    activityDidSomething: 'نفّذ إجراءً',
    activityBy: (member, action) => `${member} ${action}`,
    activitySlotWait: member => `تعذّر بدء ${member} — هناك روبوتات كثيرة تعمل في الوقت نفسه`,
    activityLabels: {
      queued: 'أرسل رسالة',
      working: 'يعمل…',
      replied: 'ردّ',
      passed: 'تخطّى',
      timedOut: 'استغرق وقتاً طويلاً',
      failed: 'واجه خطأ',
      cancelled: 'قوطعت الجولة برسالة أحدث',
      settled: 'اكتملت الجولة',
      capped: 'توقفت الجولة عند حد الجولات أو الرسائل',
      delivered: 'سلّم رداً متأخراً',
      held: 'متوقف مؤقتاً — اذكره أو أرسل resume للمتابعة',
      stopped: 'أوقف الغرفة — تبقى الجولات المتبقية معلّقة حتى الاستئناف'
    },
    stop: 'توقف',
