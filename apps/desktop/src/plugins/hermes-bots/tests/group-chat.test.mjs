...
function load(
  ...
  {
    busyUntilResumeCall,
    clarifyUntilResumeCall,
    approvalUntilResumeCall,
    conflictOnce = false,
    ...
    const slot = {
      ...
      set: value => {
        if (process.env.TRACE_ATOM && value && typeof value === 'object' && value.Grind) {
          console.error(
            'ATOM SET Grind stranded=',
            JSON.stringify(value.Grind.stranded),
...
test('a member @-mentioned by another bot joins the NEXT round', async () => {
  ...
  assert.equal(
    texts.some(t => t.startsWith('research:')),
    true
  )
  assert.equal(
    texts.some(t => t.startsWith('builder: On it')),
    true
  )
})
...
test('concurrent groups sharing one member keep sessions, deltas, and context isolated', async () => {
  ...
  assert.equal(
    alphaSession.messages.some(message => String(message.content).includes('BETA_ONLY')),
    false
  )
  assert.equal(
    betaSession.messages.some(message => String(message.content).includes('ALPHA_ONLY')),
    false
  )
})
...
test('group room messages and members mirror through bounded gateway profile metadata', async () => {
  ...
  assert.equal(
    JSON.stringify(envelope.rooms[researchKey].members.map(member => member.name)),
    JSON.stringify(['research', 'builder'])
  )
...
test('pull-before-push merge preserves disjoint rooms, messages, and members', () => {
  ...
  assert.equal(
    JSON.stringify(merged.rooms['name:Shared'].log.map(entry => entry.text)),
    JSON.stringify(['remote', 'local'])
  )
  assert.equal(
    JSON.stringify(merged.rooms['name:Shared'].members.map(member => member.name)),
    JSON.stringify(['research', 'builder'])
  )
  ...
  const stale = gc.mergeGroupChatSyncSnapshots(
    {
      version: 2,
      rooms: {
        Research: { revision: 1, log: [{ id: 'old', from: { kind: 'user', name: 'You' }, text: 'old', at: 10 }] }
      }
    },
...
test('sync worker retries a gateway CAS conflict and publishes the merged room', async () => {
  ...
  assert.equal(gc.requests.filter(call => call.method === 'profiles.configure').length, 2)
})
...
test('read-back does not clobber a newer local room edit queued during an in-flight write', () => {
  ...
  assert.equal(JSON.stringify(merged.Shared.log.map(entry => entry.id).sort()), JSON.stringify(['local', 'remote']))
})
...
test('fan-out class: a room write is mirrored to every reachable default-profile gateway', async () => {
  ...
  const configured = new Set(remoteRequests.filter(r => r.method === 'profiles.configure').map(r => r.connectionId))
...
test('source contract: group roster rows expose a confirmed Delete Group action', () => {
  ...
  assert.match(pluginSource, /children: b\.disbandGroupChat\(group\)/)
...
test('disband: a running room leaves an epoch-bumped empty tombstone so in-flight turns bail', async () => {
  ...
  assert.equal(
    gc.uniqueGroupChatName('Live', new Set(gc.liveGroupChatNames())),
    'Live',
...
test('threads: replying with an explicit thread id continues that thread and scopes the member delta to it', async () => {
  ...
  assert.ok(
    memberEntries.every(e => e.thread === billing),
    'replies land in the thread that triggered them'
  )
})
...
test('group room preview renders the bot HANDLE, not the raw profile name', () => {
  ...
  assert.match(pluginSource, /const lastHandle = botHandle\(\s*lastFrom \|\| 'bot',\s*members\.find\(/)
  assert.match(pluginSource, /\? `\$\{last\.from\?\.kind === 'user' \? b\.you : `@\$\{lastHandle\}`\}/)
