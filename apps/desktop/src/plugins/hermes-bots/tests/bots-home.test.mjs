...
test('Bot Mode copy uses localized labels', () => {
  assert.match(pluginSource, /children: taken \? b\.nameTakenForCapabilities : b\.nameFirstForCapabilities/)
  assert.match(pluginSource, /children: busy \? b\.creating : b\.createAgent/)
  assert.match(pluginSource, /b\.agentCreatedOn\(displayName\(\{ name: slug, title \}\), targetLabel\)/)
  assert.match(pluginSource, /b\.agentCreated\(displayName\(\{ name: slug, title \}\)\)/)
})
