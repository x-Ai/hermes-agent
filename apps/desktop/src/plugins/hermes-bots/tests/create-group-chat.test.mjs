...
test('source contract: header + is a dropdown offering agent and group chat', () => {
  ...
  assert.match(pluginSource, /children: \[jsx\(Codicon, \{ name: 'hubot',[\s\S]*?b\.newAgent\]/)
  assert.match(pluginSource, /children: \[jsx\(Codicon, \{ name: 'organization',[\s\S]*?b\.newGroupChat\]/)
})
