...
function useBots() {
  ...
  return new Proxy(
    ...
    {
      get: (_target, key) => {
        const fallback = BOT_COMPONENT_FALLBACKS[key]
        if (fallback !== undefined) {
          return fallback
        }
        return BOT_FUNCTION_MESSAGES.has(key) ? (...args) => args.filter(Boolean).join(' ') : String(key)
      }
    }
  )
}
      ...
      jsxs(ContextMenuContent, {
        children: [
          ...
          jsx(ContextMenuItem, {
            ...
            children: hidden ? b.unhideBot : b.hideBot
          }),
...
function routineDetailRows(job, b = BOT_COMPONENT_FALLBACKS) {
  const paused = job?.enabled === false || job?.state === 'paused'
  const label = scheduleLabel(job?.schedule, b)
  const raw = String(job?.schedule || '').trim()

  return [
    [b.status, paused ? b.paused : b.active],
    [b.schedule, label],
    // `scheduleLabel` humanizes "every 1440m" and cron expressions; keep the
    // raw string when it says something the label dropped.
    [b.scheduleRaw, raw && raw !== label ? raw : null],
    [b.repeat, job?.repeat],
    [b.nextRunLabel, paused ? null : routineTimestamp(job?.next_run_at)],
    [b.lastRun, routineTimestamp(job?.last_run_at)],
    [b.lastResult, job?.last_status],
    [b.deliversTo, job?.deliver],
    [b.model, job?.model],
    [b.workingDirectory, job?.workdir]
  ]
...
function RoutineDetailDialog({ job, onClose, open }) {
  const b = useBots()
  const rows = job ? routineDetailRows(job, b) : []
      ...
      children: [
        jsxs(DialogHeader, {
          children: [
            ...
            jsx(DialogDescription, { children: b.routineDetailDescription })
          ]
        }),
        jsxs('div', {
          className: 'grid gap-3.5',
        ...
        jsx(DialogFooter, {
          children: jsx(Button, { variant: 'secondary', onClick: onClose, children: b.close })
        })
      ]
    })
  })
          ...
          children: [
            ...
            labeled(
              ...
              pickerSelect(target, setTarget, [
                ...
                {
                  id: 'bot-chat',
                  label: b.botChatResponds(
                    displayName(
                      typeof bot === 'string' ? { name: bot } : bot,
    ...
    children: [
      ...
      ...active.map(bot => {
        ...
        return jsx(
          Tip,
          {
            label: b.openAgentChat(label),
            children: jsx('button', {
              type: 'button',
              'aria-label': b.openAgentChat(label),
              className: cn(
...
function GroupChatWorkspace({ group, members, onBack, visible = true }) {
  ...
  const header = jsxs('div', {
    ...
    children: [
      ...
      jsx(Tip, {
        label: b.disbandGroupChat(group),
        children: jsx(Button, {
          variant: 'ghost',
          size: 'sm',
          className: 'shrink-0 text-(--ui-text-tertiary) hover:text-destructive',
          'aria-label': b.disbandGroupChat(group),
        ...
        children: [
          jsxs('div', {
            ...
            children: [
              ...
              needsYou
                ? jsx(Tip, {
                    label: b.groupNeedsYouTitle,
                    children: jsx(Codicon, {
                      name: 'question',
                      className: 'shrink-0 text-(--ui-accent)',
                      'aria-label': b.needsYou
                    })
                  })
                : null,
              lastAt
