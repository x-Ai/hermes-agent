...
export const zh = defineLocale({
  ...
  connectorsPage: {
    ...
    card: {
      ...
      fact: {
        ...
        toolsSomeOn: (total, on) => `${total} 个工具，已开启 ${on} 个`
      },
      verb: {
        authenticate: '验证身份',
        connect: '连接',
    ...
    page: {
      ...
      segmentNoMatch: segment => `${segment}中没有匹配项，因此已显示全部结果`,
      freeTierNote: '登录前，连接会保留在此计算机上',
    ...
    dialog: {
      ...
      openPlugins: '打开插件标签页',
      nousLine: 'Nous 应用跟随你的账户，而不是配置档案',
      rulesReadOnly: '目前无法更改规则',
      rulesAppOff: name => `开启 ${name} 后才能更改其工具`,
      rulesSignIn: '登录后才能更改 Hermes 在此处可执行的操作',
    ...
    tools: {
      title: '工具',
      notInstalledBody: '安装到此设备后即可查看其提供的工具',
      summaryTitle: name => `Hermes 可以通过 ${name} 执行的操作`,
      summaryPreviewTitle: name => `连接后 Hermes 可以通过 ${name} 执行的操作`,
      summaryCount: count => `${count} 个工具`,
      summaryAllTools: '所有工具',
      summaryOther: '其他',
      allToolsSwitch: '开启或关闭所有工具',
      summaryAllOn: '全部开启',
      summarySomeOn: (on, total) => `已开启 ${on}/${total}`,
      summaryOff: '已关闭',
      ...
      showDetails: tool => `显示 ${tool} 的说明`,
      hideDetails: tool => `隐藏 ${tool} 的说明`,
      noMatch: '没有符合筛选条件的工具',
      loading: '正在读取工具列表',
      ...
      conflictSave: '覆盖保存',
      saveFailed: '未能保存这些工具规则',
      footerDirty: (off, backOn) => `关闭 ${off} 个工具，重新开启 ${backOn || '零'}个`,
      discard: '放弃更改',
      save: '保存更改',
      saving: '保存中…'
    },
    vocabulary: {
      facetRead: { label: '读取', long: '从此应用读取数据，不会更改内容' },
      facetWrite: { label: '写入', long: '在此应用中新建或更改内容' },
      facetDestructive: { label: '破坏性', long: '可能永久移除此应用中的内容' },
      facetUnclassified: { label: '作用未知', long: '应用未说明此工具会执行哪些操作' },
      hintReadOnly: { label: '只读', long: '该工具声明仅执行读取操作' },
      hintCreate: { label: '创建', long: '创建新内容' },
      hintUpdate: { label: '更新', long: '更改已有内容' },
      hintDelete: { label: '删除', long: '移除内容' },
      hintDestructive: { label: '破坏性', long: '此处无法撤销该工具所做的更改' },
      hintIdempotent: { label: '可重复', long: '重复运行与运行一次的结果相同' },
      hintOpenWorld: { label: '外部访问', long: '访问此应用之外的内容' }
    }
  },
  sessionImport: {
    title: '从其他应用继续',
  ...
  settings: {
    ...
    customEndpoints: {
      ...
      extraHeadersHint: '随该端点的每个请求发送（网关令牌、路由标签等），值会明文保存在 config.yaml',
      ...
      save: '保存',
      newEndpoint: '新建端点',
      reachableVia: transport => `端点可通过 ${transport} 连接`,
      apiMode: 'API 模式',
      autoDetect: '自动检测',
  ...
  updates: {
    ...
    installing: '正在安装更新',
    cantReach: '无法连接更新服务器',
    tapCheck: '点击“立即检查”以检查更新',
    updateReady: count => `有新更新可用（包含 ${count} 项更改）`,
    updateReadyUnknown: '有新更新可用',
    availableBodyRelease: tag => `版本 ${tag} 已可安装`,
    lastChecked: age => `上次检查：${age}`,
    never: '从未',
    justNow: '刚刚',
    minAgo: count => `${count} 分钟前`,
    ...
    versionDetailsBody: '此安装由应用外部管理，请使用最初的安装方式进行更新',
    versionDetailsVersion: '版本',
    versionDetailsCommit: '提交',
    versionDetailsBuildOrigin: '构建来源',
    versionDetailsBuildOriginLocal: '本地',
    versionDetailsDistribution: '分发方式',
    versionDetailsDistributionDesktop: '桌面应用',
    versionDetailsDistributionDesktopMsix: '桌面应用（MSIX）',
    versionDetailsDistributionDesktopInstaller: '桌面应用（安装程序）',
    versionDetailsDistributionSourceInstaller: '源码（安装脚本）',
    versionDetailsDistributionSourceInstallerDesktop: '源码（安装脚本）+ Hermes 桌面应用',
    versionDetailsDistributionSource: '源码',
    versionDetailsDistributionSourceDesktop: '源码 + Hermes 桌面应用',
