...
import { getRuntimeI18nLocale, LOCALE_META, TRANSLATIONS } from '@/i18n'
...
const scriptCopy = () => TRANSLATIONS[getRuntimeI18nLocale()].guidedOnboarding.script

function displayedMachineKind(): string {
  const kind = machineKind()

  return kind === 'computer' ? scriptCopy().computerKind : kind
}

export function machineForkOption(): string {
  return scriptCopy().machineSetupOption(displayedMachineKind())
}
...
export function tourOptions() {
  ...
  return { basics: copy.tourBasics, none: copy.tourNone, tour: copy.tourFull }
}

/** Locale switches and updated pill labels must not invalidate an earlier
 * decline when the handoff decides whether to show its tour. */
export function tourDeclineOptions(): Set<string> {
  return new Set([
    "I'll figure it out",
    '我自己摸索',
    ...Object.values(TRANSLATIONS).map(messages => messages.guidedOnboarding.script.tourNone)
  ])
}
...
export function forkOptions(): string[] {
  const { automate, figure, mind, skip, somethingElse } = scriptCopy()

  return machineSetupLeads()
    ? [machineForkOption(), somethingElse]
...
export function forkFallbackOptions(): string[] {
  const { automate, figure, mind, skip } = scriptCopy()
...
export function buildChatOnboardingPrompt(suggestedName?: string | null, signedIn = false): string {
  const guidedCopy = TRANSLATIONS[getRuntimeI18nLocale()].guidedOnboarding
  const copy = guidedCopy.script
  const kind = displayedMachineKind()
  const machine = machineForkOption()
  const fallback = forkFallbackOptions()
  const locale = getRuntimeI18nLocale()
  const language = locale === 'en' ? null : LOCALE_META[locale].englishName
  ...
  return [
    ...
    ...PERSONA,
    // The app language matches the greeting they already saw; a one-word name carries no language signal.
    ...(language
      ? [
          `The app is set to ${language}, so write every visible word to them in ${language} — starting now, including the option pills you place. The greeting they have already seen was in ${language} too. If they write to you in a different language, follow THEM from that point on. Everything below describes what to say, not which language to say it in; the ::onboarding and ::ask directive names, their attribute names, and the exact option values pinned below stay verbatim as provided because the app matches on them.`
        ]
      : []),
    ...
    `4. The app has just arranged itself around this chat, so offer them a look at it: one short sentence, then the line ::ask{question="${copy.tourQuestion}" options="${tour.basics}|${tour.tour}|${tour.none}"} alone as its own paragraph. Branch on the answer, then go straight to step 5 IN THE SAME TURN whichever they picked — the tour overlay has its own Done button and ending your turn on it strands them with nothing to click next.`,
    `   - "${tour.basics}": three steps, the essentials only — where their conversations live, where they ask for a job, and how to start a fresh one. Point at each and say one useful thing about it.`,
    `   - "${tour.tour}": 4 to 6 steps, a proper look around — the essentials plus whatever else the layout they just picked actually gives them.`,
    ...
    `   - "${tour.none}": no line about the tour at all, straight to step 5.`,
    ...
    `   - "${copy.skip}": say one short line that the app is theirs and this chat stays here if they ever want a hand, then stand down. No more questions, no handoff.`,
