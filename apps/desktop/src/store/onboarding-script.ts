...
/** Locale switches during onboarding must not make an earlier decline stop
 * matching when the handoff later decides whether to show the signpost. */
export function tourDeclineOptions(): Set<string> {
  return new Set(Object.values(TRANSLATIONS).map(messages => messages.guidedOnboarding.script.tourNone))
}
...
export function buildChatOnboardingPrompt(suggestedName?: string | null, signedIn = false): string {
  ...
  return [
    ...
    `4. The app has just arranged itself around this chat, so offer them a look at it: one short sentence, then the line ::ask{question="${copy.tourQuestion}" options="${tour.basics}|${tour.tour}|${tour.none}"} alone as its own paragraph. Branch on the answer, then go straight to step 5 whichever they picked.`,
    ...
    `   - "${tour.none}": one short line, and move on.`,
