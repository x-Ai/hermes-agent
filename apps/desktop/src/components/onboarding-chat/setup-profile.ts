...
export function firstTaskTitle(task: string): string {
  ...
  return trimmed.length > 28
    ? `${trimmed.slice(0, 27).trimEnd()}…`
    : trimmed || translateNow('guidedOnboarding.firstBuild')
}
...
export function buildFirstTaskRunbook(
  ...
  const copy = TRANSLATIONS[getRuntimeI18nLocale()].guidedOnboarding.script
  ...
  return [
    ...
    `When the first pass of the build is DONE: end that turn with ::ask{question="${copy.buildReviewQuestion}" options="${copy.buildReviewLooksRight}|${copy.buildReviewChange}|${copy.buildReviewFurther}"} alone as its own paragraph, emitted EXACTLY as written. Act on their pick immediately. One unreviewed first output is how a build reads as broken; the ask is how it reads as a collaboration.`,
    PLAIN_SPEECH
  ]
    .filter(Boolean)
    .join(' ')
...
function machineSetupInstructions(): string[] {
  const copy = TRANSLATIONS[getRuntimeI18nLocale()].guidedOnboarding.script
  ...
  return [
    ...
    `THEN PROPOSE, THEN ASK. Turn the gaps into a short numbered plan, cheapest and most obviously useful first: system updates, a package manager if missing, their everyday tools, sane defaults, and only then anything exotic. End that turn with ::ask{question="${copy.machineRunQuestion}" options="${copy.machineRunGoAhead}|${copy.machineRunChangeList}|${copy.machineRunEssentials}"} alone as its own paragraph, emitted EXACTLY as written.`,
...
function planRunbook(plan: HandoffPlan, pluginRoot: string, connectFirst: boolean): string[] {
  switch (plan) {
    ...
    case 'plugin':
      if (!pluginRoot) {
        throw new Error(translateNow('guidedOnboarding.errors.pluginFolderUnavailable'))
      }
...
/** Prefixes the setup instructions with machineDescription(), so the agent does not spend its first turns finding out
 *  what the app already reports. */
function machineSetupRunbook(): string[] {
  const description = machineDescription()
  const instructions = machineSetupInstructions()

  return description ? [`What the app can already see about it: ${description}.`, ...instructions] : instructions
}
...
export function buildHandoffCompleteNote(task: string): string {
  return `[setup] handoff complete — "${task.trim()}" is now building in its own session on the default profile, and the user is watching it there. The app handles the tour of the profile rail and the sessions list, respecting their earlier choice to skip it, so do not describe either. Say ONE short line and then stop: you're around if they want a hand, and this chat stays where it is. Do not ask a question, do not offer a list, do not schedule anything.`
}
