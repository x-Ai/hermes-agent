...
export function ConnectorPicks({ catalog, commit, done, locked, plugins }: ConnectorPicksProps) {
  ...
  return (
    <CardFrame
      continueLabel={pickedCount > 0 ? t.connectors.continueWith(pickedCount) : t.connectors.noneOfThese}
    ...
    >
          ...
          <div className="grid max-h-72 grid-cols-3 gap-2 overflow-y-auto">
            {shownPlugins.map(plugin => (
              <Chip
                ...
                sub={pluginNeedsApp(plugin) ? plugin.sentence : t.connectors.plugin}
              />
            ))}
            {shown.map(row => (
              <Chip
      ...
      <p className="text-xs text-muted-foreground">
        <strong className="font-medium text-foreground">{t.connectors.nothingSelectedYet}</strong>{' '}
        {t.connectors.connectOrInstallWhenNeeded}
      </p>
    </CardFrame>
  )
}
