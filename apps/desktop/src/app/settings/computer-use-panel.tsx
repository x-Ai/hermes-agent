...
function PermissionRow({ granted, label, hint }: { granted: boolean | null; label: string; hint: string }) {
  const copy = useI18n().t.settings.toolsets.computerUse
...
export function ComputerUsePanel({ onConfiguredChange }: ComputerUsePanelProps) {
  const { t } = useI18n()
  const copy = t.settings.toolsets.computerUse
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      notifyError(err, copy.statusReadFailed)
    } finally {
      setLoading(false)
    }
  }, [copy.statusReadFailed])
  ...
  const grant = useCallback(async () => {
    ...
    try {
      ...
      if (!started.ok) {
        notifyError(new Error('spawn failed'), copy.grantFailed)
  ...
  }, [copy.approveMessage, copy.approveTitle, copy.grantFailed, onConfiguredChange, refresh])
  ...
  if (!status.installed) {
    return (
      <p className="px-1 text-xs text-muted-foreground">
        {copy.installHint}
        {status.can_grant && ` ${copy.installGrantHint}`}
      </p>
    )
  }
  ...
  return (
    <div className="grid gap-2">
      <div className="flex flex-wrap items-center justify-between gap-2 px-1">
        <div className="min-w-0">
          {status.can_grant ? (
            <p className="text-[0.72rem] text-muted-foreground">{copy.macGrantNote}</p>
          ) : (
            <p className="text-[0.72rem] text-muted-foreground">{copy.platformNotes[status.platform] ?? ''}</p>
          )}
