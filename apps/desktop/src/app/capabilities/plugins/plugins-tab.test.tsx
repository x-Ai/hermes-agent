...
describe('PluginsTab', () => {
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <I18nProvider configClient={null} initialLocale="zh">
          <PluginsHarness profile={null} />
        </I18nProvider>
      </QueryClientProvider>
    )

    expect(screen.getAllByText(zh.skills.plugins.bundledNames['disk-cleanup']).length).toBeGreaterThan(0)
    expect(screen.queryByText('disk-cleanup')).toBeNull()
    expect(screen.getAllByText(zh.skills.plugins.bundledDescriptions['disk-cleanup']).length).toBeGreaterThan(0)
    expect(screen.getAllByText(zh.skills.plugins.bundledNames['security-guidance']).length).toBeGreaterThan(0)
    expect(screen.queryByText('security-guidance')).toBeNull()
    expect(screen.getAllByText(zh.skills.plugins.bundledDescriptions['security-guidance']).length).toBeGreaterThan(0)
  ...
  it('keeps a non-bundled plugin name when it matches a bundled plugin id', () => {
    ...
    expect(screen.getAllByText('disk-cleanup').length).toBeGreaterThan(0)
