...
/** ``session_id`` binds skill completions to that session's profile and workspace (project skills). ``profile`` supports discovery before a session exists, while preserving the same profile-scoped command availability rules as the catalog and dispatcher. */
