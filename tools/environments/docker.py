...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        volume_args, writable_args = self._mount_args(
            volumes, host_cwd, auto_mount_cwd, task_id, self._workspace_mount_path
        )
    ...
    def _mount_args(
        ...
        if self._persistent:
            ...
            if mount_workspace:
                ...
                writable_args += ["-v", f"{self._workspace_dir}:{workspace_mount_path}"]
        else:
            writable_args += [
                "--tmpfs", f"{workspace_mount_path}:rw,exec,size=10g"
            ] if mount_workspace else []
        ...
        if bind_host_cwd:
            ...
            volume_args = [
                "-v", f"{host_cwd_abs}:{workspace_mount_path}", *volume_args
            ]
        ...
        if state != "running":
            ...
            if err is not None:
                logger.warning(
                    ...
                    "%s — removing it before falling back to a fresh container.",
    ...
    def _wait_for_racing_container(
        ...
        *, attempts: int = 50, delay: float = 0.1,
        ...
        for attempt in range(attempts):
            ...
            if existing is not None and existing[1] == "running":
                return existing[0]
    ...
    def _run_new_container(
        self, run_cmd: list[str], *, container_name: str,
        task_label: str, profile_label: str, egress_label: str,
        race_safe: bool,
    ) -> tuple[str, bool]:
        try:
            result = run_capture(
                run_cmd, timeout=120, check=True,
