...
def _build_child_system_prompt(
    goal: str, context: Optional[str] = None, *, workspace_path: Optional[str] = None,
    context_workspace_path: Any = _WORKSPACE_CONTEXT_UNSET, role: str = "leaf",
        ...
        with _quiet("subagent: workspace context-files load failed", exc_info=True):
            ...
            if context_workspace:
                _ctx_files = build_context_files_prompt(
                    cwd=str(context_workspace), skip_soul=True
                )
        if _ctx_files.strip():
...
def _resolve_workspace_hint(parent_agent) -> Optional[str]:
    ...
    with _quiet("subagent: execution workspace resolution failed", exc_info=True):
