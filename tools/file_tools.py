...
def _create_terminal_env_for_file_ops(raw_task_id: str, task_id: str):
    ...
    from tools.terminal_tool import (
        ...
        _resolve_task_host_cwd, _resolve_workspace_mount_for_task, _select_image,
        get_session_cwd, resolve_task_overrides)
    ...
    if _is_container_backend(env_type) and _is_unusable_container_cwd(cwd):
        remapped = (
            config.get("workspace_mount_path") or "/workspace"
    ...
    terminal_env = _create_configured_env(
        ...
        host_cwd=(
            host_cwd if mount_source
            else _resolve_task_host_cwd(config, raw_task_id)
        ),
